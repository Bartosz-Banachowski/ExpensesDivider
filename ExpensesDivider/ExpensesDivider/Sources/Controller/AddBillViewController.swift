...
class AddBillViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func setupWhoPaidPicker() {
        ...
        whoPaidArray.append(Friend(username: "You", email: Auth.auth().currentUser!.email!)!)
    ...
    @objc func saveBill() {
        ...
        if let message = error {
            ...
            let newBill = Bill(description: description!, money: Decimal(string: money)!, date: date!, whoPaid: whoPaid!, debtorList: debtorList)
